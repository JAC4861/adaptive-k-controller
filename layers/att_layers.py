...
#         h_prime = h_prime.div(e_rowsum + 1e-12)
#         # h_prime: N x out
#         assert not torch.isnan(h_prime).any()
...
class SpGraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        edge_logits = self.a.mm(edge_h).squeeze()

        # ======[ THE FIX IS HERE / 修正就在这里 ]======
        # Clamp the logits to a reasonable range before applying exp to prevent overflow.
        # 在送入 exp() 前，将 logits 的值截断在一个合理范围内，防止溢出成 inf。
        edge_logits = torch.clamp(edge_logits, -10, 10)
        # ===============================================

        edge_e = torch.exp(-self.leakyrelu(edge_logits))
        
        ones = torch.ones(size=(N, 1), device=h.device)
        e_rowsum = self.special_spmm(edge, edge_e, torch.Size([N, N]), ones)
