...
class GCN(Encoder):
    ...
    def __init__(self, c, args):
        ...
        for i in range(len(dims) - 1):
            in_dim, out_dim = dims[i], dims[i + 1]
            act = acts[i]
            # 这里的参数与你 layers.py 中的 GraphConvolution 构造函数一致
            self.layers.append(GraphConvolution(in_dim, out_dim, args.dropout, act, args.bias))

    def encode(self, x, adj):
        # 重写 encode 方法，确保数据流正确
        for layer in self.layers:
            # 1. 将 x 和 adj 打包成元组传入
            # 2. layer 现在只返回一个值，所以我们只用 x 来接收
            x = layer((x, adj))
            # 3. 移除此处的激活函数，因为它已在 layer 内部完成
        return x
...
class HGCN(Encoder):
    ...
    def __init__(self, c, args):
        ...
        self.manifold = getattr(manifolds, args.manifold)()
        print(f"DEBUG: Manifold object is: {self.manifold}") # <--- 添加这一行
        assert args.num_layers > 1
