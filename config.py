...
config_args = {
    'training_config': {
        ...
        'log-freq': (20, 'print train metrics every N epochs'),
        'eval-freq': (20, 'evaluate every N epochs'),
        'eval-test-freq': (0, 'compute test metrics every N evaluation events; 0=skip periodic test'),
        ...
        'data-root': ('./data', 'root directory for datasets (including OGB)'),
        'eval-metric': ('auto', 'evaluation metric: auto|hits@50|mrr|roc_ap'),
        'batch-size-edge-eval': (65536, 'edge batch size for OGB eval/prediction'),
        'num-neg-train': (1, 'negative samples per positive edge in OGB train'),
        'use-edge-attr': (0, '1=enable edge attributes when model supports it'),
        'use-valedges-as-input': (0, 'OGB trick flag, default off for clean protocol'),
        ...
        'distortion-monitor-every': (1, 'monitor distortion every N eval events'),
...
parser.add_argument('--distortion-pairs-limit', type=int, default=8000,
