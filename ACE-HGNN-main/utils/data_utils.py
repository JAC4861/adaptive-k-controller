...
def load_data(args, datapath):
    ...
    if args.task == 'nc':
        ...
        if args.task == 'lp':
            start = time.time()
            split_path = getattr(args, 'edge_split_path', None)
            loaded_from = None
            if split_path:
                adj_train, train_edges, train_edges_false, val_edges, val_edges_false, test_edges, test_edges_false = load_lp_edge_split(
                        split_path, adj.shape
                )
                loaded_from = split_path
            else:
                cache_path = get_lp_edge_split_cache_path(datapath, args.val_prop, args.test_prop, args.split_seed)
                use_cache = int(getattr(args, 'cache_lp_splits', 1)) == 1
                if use_cache and os.path.exists(cache_path):
                    adj_train, train_edges, train_edges_false, val_edges, val_edges_false, test_edges, test_edges_false = load_lp_edge_split(
                            cache_path, adj.shape
                    )
                    loaded_from = cache_path
                else:
                    ...
                    if use_cache:
                        ...
                        loaded_from = cache_path
            if split_path:
                print("Edge split loaded from {}...time: {}s.".format(loaded_from, time.time() - start))
            elif loaded_from and os.path.exists(loaded_from):
                print("Edge split ready from {}...time: {}s.".format(loaded_from, time.time() - start))
            else:
...
def load_paths(dataset, data_path):
    ...
    paths_dict = np.load(dict_path, allow_pickle=True).item()
    global PATHDICT
    PATHDICT = paths_dict
    end = time.time()
...
def get_lp_edge_split_cache_path(data_path, val_prop, test_prop, seed):
    def fmt_prop(prop):
        return format(prop, '.6g').replace('.', 'p')

    filename = 'lp_split_seed{}_val{}_test{}.npz'.format(seed, fmt_prop(val_prop), fmt_prop(test_prop))
    return os.path.join(data_path, filename)


def build_adj_from_edges(train_edges, shape):
    adj_train = sp.csr_matrix((np.ones(train_edges.shape[0]), (train_edges[:, 0], train_edges[:, 1])), shape=shape)
    return adj_train + adj_train.T


def tensor_to_numpy(edges):
    if torch.is_tensor(edges):
        edges = edges.cpu().numpy()
    return np.asarray(edges, dtype=np.int64)


def save_lp_edge_split(path, train_edges, train_edges_false, val_edges, val_edges_false, test_edges, test_edges_false):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    np.savez_compressed(
            path,
            train_edges=tensor_to_numpy(train_edges),
            train_edges_false=tensor_to_numpy(train_edges_false),
            val_edges=tensor_to_numpy(val_edges),
            val_edges_false=tensor_to_numpy(val_edges_false),
            test_edges=tensor_to_numpy(test_edges),
            test_edges_false=tensor_to_numpy(test_edges_false),
    )


def load_lp_edge_split(path, adj_shape):
    split_data = np.load(path)
    train_edges = np.asarray(split_data['train_edges'], dtype=np.int64)
    train_edges_false = np.asarray(split_data['train_edges_false'], dtype=np.int64)
    val_edges = np.asarray(split_data['val_edges'], dtype=np.int64)
    val_edges_false = np.asarray(split_data['val_edges_false'], dtype=np.int64)
    test_edges = np.asarray(split_data['test_edges'], dtype=np.int64)
    test_edges_false = np.asarray(split_data['test_edges_false'], dtype=np.int64)
    adj_train = build_adj_from_edges(train_edges, adj_shape)
    return adj_train, torch.LongTensor(train_edges), torch.LongTensor(train_edges_false), torch.LongTensor(val_edges), \
           torch.LongTensor(val_edges_false), torch.LongTensor(test_edges), torch.LongTensor(test_edges_false)


def sample_negative_edges(num_nodes, excluded_edges, num_samples, seed):
    ...
    while len(sampled_edges) < num_samples:
        rows = rng.randint(0, num_nodes, size=batch_size)
        cols = rng.randint(0, num_nodes, size=batch_size)
        valid = rows < cols
        for row, col in zip(rows[valid], cols[valid]):
            edge = (int(row), int(col))
            if edge in excluded_edges or edge in sampled_set:
                continue
...
def mask_edges(adj, val_prop, test_prop, seed):
    rng = np.random.RandomState(seed)
    adj_triu = sp.triu(adj, k=1).tocoo()
    pos_edges = np.column_stack((adj_triu.row, adj_triu.col)).astype(np.int64, copy=False)
    ...
    pos_edge_set = {tuple(edge) for edge in pos_edges.tolist()}
    neg_edges = sample_negative_edges(adj.shape[0], pos_edge_set, n_train + n_val + n_test, seed)
    val_edges_false = neg_edges[:n_val]
    test_edges_false = neg_edges[n_val:n_val + n_test]
    train_edges_false = neg_edges[n_val + n_test:]
    adj_train = build_adj_from_edges(train_edges, adj.shape)
    return adj_train, torch.LongTensor(train_edges), torch.LongTensor(train_edges_false), torch.LongTensor(val_edges), \
